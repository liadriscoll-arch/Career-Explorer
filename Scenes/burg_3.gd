...
func _on_assemblyarea_thirdburg() -> void:
	...
	Chefglobal.inventory == "Cheese"):
		if Chefglobal.inventory in Chefglobal.asemburgtop3:
			alron.emit()
		elif Chefglobal.asemburgtop3.size() > 3:
			tomuch.emit()
		else:
			Chefglobal.asemburgtop3.append(Chefglobal.inventory)
			Chefglobal.inventory = ""
	elif isready and Chefglobal.inventory == "":
		visible = false
		toppings = ", ".join(Chefglobal.asemburgtop3) 
		Chefglobal.inventory= buntype + " " + burgtype + " with " + toppings
		if Chefglobal.asemburgtop3 == []:
			Chefglobal.inventory = buntype + " " + burgtype
		Chefglobal.asemburgtop3.clear()
		toppings = ""
		isready = false
